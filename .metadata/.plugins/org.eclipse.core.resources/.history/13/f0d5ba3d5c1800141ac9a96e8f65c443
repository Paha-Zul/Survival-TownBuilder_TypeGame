...
class InventoryScala[E](owner : Entity, name : String, compType : Int, active : Boolean, 
	...
	def removeItemFromInventory(quantity : Int, 
			criteria : InventoryItemScala[E] => Boolean) : InventoryItemScala[E] = {
	...
	def hasItemInInventory(itemToFind : E, criteria : (InventoryItemScala[E],InventoryItemScala[E]) => Boolean) : InventoryItemScala[E] = {
		val _invItemToFind = new InventoryItemScala[E](itemToFind, 1);
		
		for(invItem : InventoryItemScala[E] <- this.inventory)
			if(criteria(_invItemToFind, invItem))
				return invItem;
	...
	/**
	 * Takes in a criteria for an item in this inventory to satisfy.
	 * Returns the True if there was an item that satisfied this criteria, false otheriwse.
	 */
	def hasItemInInventory(criteria : (InventoryItemScala[E]) => Boolean) : InventoryItemScala[E] = {
		for(invItem : InventoryItemScala[E] <- this.inventory)
			if(criteria(invItem))
				return invItem;
